...
class ViewModel: ObservableObject {
    ...
    func loadFromJSON(){
        if let data = try? Data(contentsOf: jsonURL),
           let decoded = try? JSONDecoder().decode([Memory].self, from: data){ memories = decoded.sorted()
        }
    }
    ...
    // ❗️
    // Find the memory in the list that matches the edited one, replace it with the new version, then save everything.
...
struct Memory: Identifiable, Comparable, Hashable, Codable {
    ...
    static func <(lhs: Memory, rhs: Memory) -> Bool { lhs.name < rhs.name }
}
